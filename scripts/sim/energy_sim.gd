...
static func demand_of(gs, b: Dictionary) -> float:
	# Libre mercado: las empresas NPC también consumen (pagan desde su caja; acometida la paga su dueño).
	if not (gs.owned_by_player(b) or NpcBusinessSim.is_npc(b)) or b["status"] != "activo" or not BusinessSim.is_business(b):
		return 0.0
